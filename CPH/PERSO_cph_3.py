#!/usr/bin/env python3
# -*- coding: utf-8 -*-

import os
import json
import re
import unicodedata
import hashlib
import pickle
import time
import traceback
import argparse
import sys
from collections import Counter
from datetime import datetime
from typing import Dict, Tuple, Optional

"""
OCR structuré + Analyse juridique (Ollama) - VERSION AVEC ANONYMISATION
- Démarrage sécurisé et gestion d'erreurs améliorée
- System prompt envoyé séparément
- Nettoyage amélioré des numéros de page
- Gestion prompts JSON corrigée
- OCR indépendant de l'analyse automatique
- Système de cache OCR intelligent
- ANONYMISATION automatique des résultats
"""


# =================================
#  SYSTÈME D'ANONYMISATION
# =================================

class TextAnonymizer:
  """Classe pour anonymiser les textes juridiques"""
  
  def __init__(self, strict_mode: bool = True):
    self.strict_mode = strict_mode
    self.replacement_map = {}
    self.counters = {
      'person': 0,
      'company': 0,
      'amount': 0,
      'date': 0,
      'address': 0
    }
  
  def anonymize_text(self, text: str) -> str:
    """Anonymise un texte complet"""
    if not text:
      return text
    
    result = text
    
    # Anonymisation des montants monétaires
    result = self._anonymize_amounts(result)
    
    # Anonymisation des dates
    result = self._anonymize_dates(result)
    
    # Anonymisation des noms et prénoms
    result = self._anonymize_names(result)
    
    # Anonymisation des entreprises
    result = self._anonymize_companies(result)
    
    # Anonymisation des adresses
    result = self._anonymize_addresses(result)
    
    # Anonymisation des numéros de téléphone
    result = self._anonymize_phone_numbers(result)
    
    # Anonymisation des emails
    result = self._anonymize_emails(result)
    
    return result
  
  def _anonymize_amounts(self, text: str) -> str:
    """Anonymise les montants monétaires"""
    patterns = [
      r'\b\d+(?:\s*\d{3})*(?:[.,]\d{2})?\s*(?:euros?|€|EUR)\b',
      r'\b(?:euros?|€|EUR)\s*\d+(?:\s*\d{3})*(?:[.,]\d{2})?\b',
      r'\b\d+(?:\s*\d{3})*(?:[.,]\d{2})?\s*(?:francs?|F|FF)\b',
      r'\b\d+(?:[.,]\d{2})?\s*(?:centimes?|c)\b',
      r'\b(?:la\s+somme\s+de|le\s+montant\s+de|pour\s+un\s+montant\s+de)\s+\d+(?:\s*\d{3})*(?:[.,]\d{2})?\s*(?:euros?|€|EUR)\b'
    ]
    
    for pattern in patterns:
      text = re.sub(pattern, '[MONTANT_ANONYME]', text, flags=re.IGNORECASE)
    
    return text
  
  def _anonymize_dates(self, text: str) -> str:
    """Anonymise les dates"""
    patterns = [
      r'\b\d{1,2}[/.-]\d{1,2}[/.-]\d{2,4}\b',
      r'\b\d{1,2}\s+(?:janvier|février|mars|avril|mai|juin|juillet|août|septembre|octobre|novembre|décembre)\s+\d{2,4}\b',
      r'\b(?:lundi|mardi|mercredi|jeudi|vendredi|samedi|dimanche)\s+\d{1,2}\s+(?:janvier|février|mars|avril|mai|juin|juillet|août|septembre|octobre|novembre|décembre)\s+\d{2,4}\b',
      r'\b\d{2,4}[/.-]\d{1,2}[/.-]\d{1,2}\b'
    ]
    
    for pattern in patterns:
      text = re.sub(pattern, '[DATE_ANONYME]', text, flags=re.IGNORECASE)
    
    return text
  
  def _anonymize_names(self, text: str) -> str:
    """Anonymise les noms et prénoms"""
    # Patterns pour détecter les noms
    name_patterns = [
      r'\b(?:M\.|Mme|Monsieur|Madame|Mlle|Mademoiselle)\s+([A-Z][a-zàâäéèêëïîôùûüÿç]+(?:\s+[A-Z][a-zàâäéèêëïîôùûüÿç]+)*)\b',
      r'\b([A-Z][A-Z]+)\s+([A-Z][a-zàâäéèêëïîôùûüÿç]+)\b',
      r'\bde\s+([A-Z][a-zàâäéèêëïîôùûüÿç]+)\b',
      r'\b([A-Z][a-zàâäéèêëïîôùûüÿç]+)\s+né[e]?\s+([A-Z][a-zàâäéèêëïîôùûüÿç]+)\b'
    ]
    
    # Remplacement par des codes anonymes
    for pattern in name_patterns:
      matches = re.finditer(pattern, text, re.IGNORECASE)
      for match in matches:
        full_match = match.group(0)
        if full_match not in self.replacement_map:
          self.counters['person'] += 1
          self.replacement_map[full_match] = f"[PERSONNE_{self.counters['person']}]"
        text = text.replace(full_match, self.replacement_map[full_match])
    
    return text
  
  def _anonymize_companies(self, text: str) -> str:
    """Anonymise les noms d'entreprises"""
    company_patterns = [
      r'\b[A-Z][A-Za-zàâäéèêëïîôùûüÿç\s]+\s+(?:SA|SARL|SAS|SASU|SNC|SCA|EURL|GIE|SCOP|SEM)\b',
      r'\b(?:SA|SARL|SAS|SASU|SNC|SCA|EURL|GIE|SCOP|SEM)\s+[A-Z][A-Za-zàâäéèêëïîôùûüÿç\s]+\b',
      r'\b(?:Société|Entreprise|Compagnie|Établissements?)\s+[A-Z][A-Za-zàâäéèêëïîôùûüÿç\s]+\b',
      r'\b[A-Z][A-Za-zàâäéèêëïîôùûüÿç\s]+\s+(?:& Cie|et Fils|France|International|Group[e]?)\b'
    ]
    
    for pattern in company_patterns:
      matches = re.finditer(pattern, text)
      for match in matches:
        full_match = match.group(0)
        if full_match not in self.replacement_map:
          self.counters['company'] += 1
          self.replacement_map[full_match] = f"[SOCIETE_{self.counters['company']}]"
        text = text.replace(full_match, self.replacement_map[full_match])
    
    return text
  
  def _anonymize_addresses(self, text: str) -> str:
    """Anonymise les adresses"""
    address_patterns = [
      r'\b\d+[,\s]+(?:rue|avenue|boulevard|place|impasse|allée|chemin|route)\s+[A-Za-zàâäéèêëïîôùûüÿç\s]+\b',
      r'\b\d{5}\s+[A-Z][A-Za-zàâäéèêëïîôùûüÿç\s-]+\b',
      r'\b(?:domicilié[e]?\s+)?(?:au|à|en)\s+\d+[,\s]+(?:rue|avenue|boulevard|place|impasse|allée|chemin|route)\s+[A-Za-zàâäéèêëïîôùûüÿç\s]+\b'
    ]
    
    for pattern in address_patterns:
      text = re.sub(pattern, '[ADRESSE_ANONYME]', text, flags=re.IGNORECASE)
    
    return text
  
  def _anonymize_phone_numbers(self, text: str) -> str:
    """Anonymise les numéros de téléphone"""
    phone_patterns = [
      r'\b0[1-9](?:[.\-\s]?\d{2}){4}\b',
      r'\b\+33[.\-\s]?[1-9](?:[.\-\s]?\d{2}){4}\b'
    ]
    
    for pattern in phone_patterns:
      text = re.sub(pattern, '[TELEPHONE_ANONYME]', text)
    
    return text
  
  def _anonymize_emails(self, text: str) -> str:
    """Anonymise les adresses email"""
    email_pattern = r'\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}\b'
    return re.sub(email_pattern, '[EMAIL_ANONYME]', text)
  
  def get_anonymization_report(self) -> str:
    """Génère un rapport d'anonymisation"""
    total = sum(self.counters.values()) + len(self.replacement_map)
    
    if total == 0:
      return "Aucune donnée sensible détectée dans le texte."
    
    report_parts = ["=== RAPPORT D'ANONYMISATION ==="]
    report_parts.append(f"Total d'éléments anonymisés : {total}")
    
    for key, count in self.counters.items():
      if count > 0:
        labels = {
          'person': 'Personnes',
          'company': 'Sociétés', 
          'amount': 'Montants',
          'date': 'Dates',
          'address': 'Adresses'
        }
        report_parts.append(f"- {labels.get(key, key)} : {count}")
    
    if self.replacement_map:
      report_parts.append(f"- Noms/entités mappées : {len(self.replacement_map)}")
    
    report_parts.append("=" * 35)
    
    return "\n".join(report_parts)


# Fonction globale d'anonymisation
def anonymize_legal_text(text: str, strict_mode: bool = True) -> Tuple[str, str]:
  """
  Anonymise un texte juridique et retourne le texte + rapport
  
  Args:
    text: Le texte à anonymiser
    strict_mode: Mode d'anonymisation strict
  
  Returns:
    Tuple (texte_anonymisé, rapport_anonymisation)
  """
  anonymizer = TextAnonymizer(strict_mode=strict_mode)
  anonymized_text = anonymizer.anonymize_text(text)
  report = anonymizer.get_anonymization_report()
  
  return anonymized_text, report




# VÃ©rification des dÃ©pendances au dÃ©marrage
def check_dependencies():
    """VÃ©rifie que toutes les dÃ©pendances sont installÃ©es"""
    missing = []
    
    try:
        import gradio as gr
    except ImportError:
        missing.append("gradio")
    
    try:
        import requests
    except ImportError:
        missing.append("requests")
    
    try:
        from pdf2image import convert_from_path
    except ImportError:
        missing.append("pdf2image")
    
    try:
        import pytesseract
    except ImportError:
        missing.append("pytesseract")
    
    if missing:
        print(f"Erreur: DÃ©pendances manquantes: {', '.join(missing)}")
        print("Installez avec: pip install " + " ".join(missing))
        sys.exit(1)

# VÃ©rifier les dÃ©pendances avant d'importer
check_dependencies()

import gradio as gr
import requests
from pdf2image import convert_from_path
import pytesseract
import os
import json
import re
import unicodedata
import hashlib
import pickle
import time
import traceback
import argparse
import sys
from collections import Counter
from datetime import datetime
from typing import Dict, Tuple, Optional
import os
import json
import re
import unicodedata
import hashlib
import pickle
import time
import traceback
import argparse
import sys
from collections import Counter
from datetime import datetime
from typing import Dict, Tuple, Optional, Set

# =======================
#  PARAMÃˆTRES & PROMPTS
# =======================

DEFAULT_PROMPT_NAME = "Par dÃ©faut (analyse rÃ©digÃ©e)"
DEFAULT_PROMPT_TEXT = """Tu es juriste spÃ©cialisÃ© en droit du travail. Ã€ partir du texte fourni, rÃ©dige une analyse juridique complÃ¨te en franÃ§ais juridique, en paragraphes continus (sans listes ni numÃ©rotation), visant Ã  faire ressortir les moyens de droit (arguments juridiques) pertinents, tels qu'ils ressortent exclusivement du document.

Exigences impÃ©ratives :
- Ne JAMAIS inventer ni supposer des faits ou des rÃ©fÃ©rences. Si une information n'apparaÃ®t pas dans le texte, Ã©cris : Â« non prÃ©cisÃ© dans le document Â».
- Si le texte contient une rÃ©fÃ©rence manifestement erronÃ©e (ex. confusion entre Code du travail et Code civil), signale-le explicitement sans inventer un numÃ©ro d'article.
- Anonymisation stricte : remplace les noms par [Mme X], [M. Y], [SociÃ©tÃ© Z] et les montants par [montant].

Structure implicite attendue (sans titres apparents) :
1. Qualification juridique des faits et rappel du contexte procÃ©dural (uniquement si mentionnÃ©).
2. ExposÃ© des moyens des parties : fondements, arguments, contestations, en citant uniquement ce qui figure dans le texte.
3. RÃ¨gles de droit applicables : uniquement celles prÃ©sentes dans le texte ; sinon indiquer Â« non prÃ©cisÃ© dans le document Â».
4. Discussion : articulation arguments/rÃ¨gles, charge de la preuve, incidences procÃ©durales si mentionnÃ©es.
5. Application au cas d'espÃ¨ce.
6. Conclusion motivÃ©e sur la portÃ©e des moyens (sans se substituer au juge).

RÃ©ponds uniquement par l'analyse rÃ©digÃ©e, sans commentaires mÃ©ta ni hypothÃ¨ses."""

EXPERT_PROMPT_TEXT = """Tu es un juriste senior en droit du travail. RÃ©dige une analyse approfondie des moyens de droit en franÃ§ais juridique, SANS listes ni numÃ©rotation. Mets en Ã©vidence : (i) la qualification prÃ©cise des faits, (ii) l'articulation des moyens principaux et subsidiaires, (iii) le lien exact avec les rÃ©fÃ©rences textuelles prÃ©sentes DANS le document UNIQUEMENT (si une rÃ©fÃ©rence manque : Â« non prÃ©cisÃ© dans le document Â»), (iv) la charge de la preuve et les incidences procÃ©durales si le texte en fait Ã©tat, (v) une conclusion motivÃ©e. Aucune invention, aucune hypothÃ¨se."""

SYSTEM_PROMPT = """Tu es juriste en droit du travail. RÃ©dige une analyse argumentÃ©e en franÃ§ais juridique, sans listes Ã  puces, en citant uniquement les fondements mentionnÃ©s dans le texte."""

def _script_dir() -> str:
    """DÃ©termine le rÃ©pertoire du script de maniÃ¨re sÃ©curisÃ©e"""
    try:
        if hasattr(__file__, '__file__'):
            return os.path.dirname(os.path.abspath(__file__))
        else:
            return os.getcwd()
    except:
        return os.getcwd()

def _default_store_dir() -> str:
    """CrÃ©e le rÃ©pertoire de stockage des prompts"""
    script_dir = _script_dir()
    prompts_dir = os.path.join(script_dir, "prompts")
    
    try:
        os.makedirs(prompts_dir, exist_ok=True)
        # Test d'Ã©criture
        test_path = os.path.join(prompts_dir, ".write_test")
        with open(test_path, "w", encoding="utf-8") as f:
            f.write("ok")
        os.remove(test_path)
        return prompts_dir
    except Exception as e:
        print(f"Attention: Impossible de crÃ©er ./prompts : {e}")
        return script_dir

# Configuration globale
try:
    PROMPT_STORE_DIR = _default_store_dir()
    PROMPT_STORE_PATH = os.path.join(PROMPT_STORE_DIR, "prompts_store.json")
    PROMPT_FILES_DIR = PROMPT_STORE_DIR
    print(f"RÃ©pertoire des prompts : {PROMPT_STORE_DIR}")
except Exception as e:
    print(f"Erreur configuration rÃ©pertoires : {e}")
    sys.exit(1)

# =================================
#  SYSTÃˆME DE CACHE OCR
# =================================

def get_pdf_hash(pdf_path: str) -> Optional[str]:
    """Calcule un hash du PDF pour identifier les fichiers de maniÃ¨re unique"""
    if not pdf_path or not os.path.exists(pdf_path):
        return None
    
    try:
        with open(pdf_path, 'rb') as f:
            hash_obj = hashlib.md5()
            for chunk in iter(lambda: f.read(4096), b""):
                hash_obj.update(chunk)
        return hash_obj.hexdigest()
    except Exception as e:
        print(f"Erreur calcul hash PDF : {e}")
        return None

def get_cache_path(pdf_hash: str, nettoyer: bool) -> str:
    """GÃ©nÃ¨re le chemin du fichier cache"""
    cache_dir = os.path.join(_script_dir(), "cache_ocr")
    os.makedirs(cache_dir, exist_ok=True)
    
    suffix = "_clean" if nettoyer else "_raw"
    return os.path.join(cache_dir, f"ocr_{pdf_hash}{suffix}.pkl")

def save_ocr_cache(pdf_hash: str, nettoyer: bool, ocr_data: dict) -> bool:
    """Sauvegarde le rÃ©sultat OCR en cache"""
    try:
        cache_path = get_cache_path(pdf_hash, nettoyer)
        with open(cache_path, 'wb') as f:
            pickle.dump(ocr_data, f)
        print(f"OCR mis en cache : {cache_path}")
        return True
    except Exception as e:
        print(f"Erreur sauvegarde cache : {e}")
        return False

def load_ocr_cache(pdf_hash: str, nettoyer: bool) -> Optional[dict]:
    """Charge le rÃ©sultat OCR depuis le cache"""
    try:
        cache_path = get_cache_path(pdf_hash, nettoyer)
        if os.path.exists(cache_path):
            with open(cache_path, 'rb') as f:
                data = pickle.load(f)
            print(f"OCR chargÃ© depuis le cache : {cache_path}")
            return data
    except Exception as e:
        print(f"Erreur chargement cache : {e}")
    return None

def clear_ocr_cache() -> int:
    """Nettoie le cache OCR et retourne le nombre de fichiers supprimÃ©s"""
    try:
        cache_dir = os.path.join(_script_dir(), "cache_ocr")
        if not os.path.exists(cache_dir):
            return 0
        
        count = 0
        for filename in os.listdir(cache_dir):
            if filename.startswith("ocr_") and filename.endswith(".pkl"):
                os.remove(os.path.join(cache_dir, filename))
                count += 1
        return count
    except Exception as e:
        print(f"Erreur nettoyage cache : {e}")
        return 0

# =================================
#  GESTIONNAIRE DE FICHIERS PROMPTS TXT
# =================================

def get_prompt_files(directory: str) -> list:
    """RÃ©cupÃ¨re la liste des fichiers prompt*.txt dans le rÃ©pertoire"""
    try:
        if not os.path.exists(directory):
            return []
        
        files = []
        for filename in os.listdir(directory):
            if filename.startswith("prompt") and filename.endswith(".txt"):
                filepath = os.path.join(directory, filename)
                if os.path.isfile(filepath):
                    files.append(filename)
        
        return sorted(files)
    except Exception as e:
        print(f"Erreur lecture rÃ©pertoire prompts : {e}")
        return []

def read_prompt_file(filepath: str) -> str:
    """Lit le contenu d'un fichier prompt"""
    try:
        with open(filepath, 'r', encoding='utf-8') as f:
            return f.read().strip()
    except Exception as e:
        return f"Erreur lecture fichier {filepath}: {e}"

def concatenate_prompts(selected_files: list, directory: str) -> str:
    """ConcatÃ¨ne les prompts sÃ©lectionnÃ©s"""
    if not selected_files:
        return "Aucun fichier sÃ©lectionnÃ©."
    
    result_parts = []
    result_parts.append("=== PROMPT GÃ‰NÃ‰RAL COMPOSÃ‰ ===\n")
    
    for filename in selected_files:
        filepath = os.path.join(directory, filename)
        content = read_prompt_file(filepath)
        
        result_parts.append(f"--- {filename} ---")
        result_parts.append(content)
        result_parts.append("")
    
    result_parts.append("=== FIN PROMPT COMPOSÃ‰ ===")
    return "\n".join(result_parts)

# =================================
#  GESTIONNAIRE DE PROMPTS (JSON)
# =================================

def atomic_write_text(path: str, data: str) -> None:
    """Ã‰criture atomique avec gestion d'erreur amÃ©liorÃ©e"""
    tmp = path + ".tmp"
    try:
        with open(tmp, "w", encoding="utf-8") as f:
            f.write(data)
        os.replace(tmp, path)
    except Exception:
        if os.path.exists(tmp):
            try:
                os.remove(tmp)
            except:
                pass
        raise

def load_prompt_store() -> Dict[str, str]:
    """Charge le fichier JSON des prompts"""
    default_store = {DEFAULT_PROMPT_NAME: DEFAULT_PROMPT_TEXT}
    
    if not os.path.exists(PROMPT_STORE_PATH):
        try:
            os.makedirs(os.path.dirname(PROMPT_STORE_PATH), exist_ok=True)
            atomic_write_text(PROMPT_STORE_PATH, json.dumps(default_store, ensure_ascii=False, indent=2))
            return default_store
        except Exception as e:
            print(f"Attention: Impossible de crÃ©er le store initial: {e}")
            return default_store
    
    try:
        with open(PROMPT_STORE_PATH, "r", encoding="utf-8") as f:
            store = json.load(f)
        
        if not isinstance(store, dict) or not store:
            return default_store
            
        if DEFAULT_PROMPT_NAME not in store:
            store[DEFAULT_PROMPT_NAME] = DEFAULT_PROMPT_TEXT
            save_prompt_store(store)
            
        return store
        
    except (json.JSONDecodeError, Exception) as e:
        print(f"Erreur chargement store: {e}")
        return default_store

def save_prompt_store(store: Dict[str, str]) -> Tuple[bool, str]:
    """Sauvegarde le store"""
    try:
        if not isinstance(store, dict):
            return False, "Erreur: store n'est pas un dictionnaire"
        
        os.makedirs(os.path.dirname(PROMPT_STORE_PATH), exist_ok=True)
        json_data = json.dumps(store, ensure_ascii=False, indent=2)
        atomic_write_text(PROMPT_STORE_PATH, json_data)
        
        return True, f"EnregistrÃ© dans : `{PROMPT_STORE_PATH}`"
        
    except Exception as e:
        return False, f"Ã‰chec d'enregistrement : {e}"

def sanitize_name(name: str) -> str:
    """Nettoie et valide un nom de prompt"""
    if not name:
        return ""
    name = re.sub(r"\s+", " ", name.strip())
    name = re.sub(r'[<>:"/\\|?*]', "_", name)
    return name[:100]

# ================
#  OCR & NETTOYAGE
# ================

def _normalize_unicode(text: str) -> str:
    if not text:
        return text
    text = unicodedata.normalize("NFC", text)
    replacements = {
        "Ã¯Â¬": "fi", "Ã¯Â¬â€š": "fl", "Ã¯Â¬â‚¬": "ff", "Ã¯Â¬Æ’": "ffi", "Ã¯Â¬â€ž": "ffl",
        "'": "'", "'": "'", """: '"', """: '"',
        "â€“": "-", "â€”": "-", "â€¦": "...",
    }
    for old, new in replacements.items():
        text = text.replace(old, new)
    return text

def _strip_headers_footers(pages_lines):
    """DÃ©tecte/supprime en-tÃªtes/pieds rÃ©pÃ©tÃ©s"""
    if not pages_lines or len(pages_lines) < 2:
        return pages_lines
        
    counter = Counter()
    
    for lines in pages_lines:
        if not lines:
            continue
        sample = lines[:3] + (lines[-3:] if len(lines) > 6 else [])
        
        for line in sample:
            clean_line = line.strip()
            if len(clean_line) >= 3 and not clean_line.isdigit():
                counter.update([clean_line])
    
    min_occurrences = max(2, len(pages_lines) // 3)
    repeated = {l for l, c in counter.items() if c >= min_occurrences}
    
    cleaned_pages = []
    for lines in pages_lines:
        new_lines = [line for line in lines if line.strip() not in repeated]
        cleaned_pages.append(new_lines)
    
    return cleaned_pages

def smart_clean(text: str, pages_texts=None) -> str:
    """Nettoyage amÃ©liorÃ© des artÃ©facts OCR"""
    if not text:
        return text

    text = _normalize_unicode(text)

    if pages_texts:
        pages_lines = [t.splitlines() for t in pages_texts]
        pages_lines = _strip_headers_footers(pages_lines)
        text = "\n".join("\n".join(lines) for lines in pages_lines)

    text = re.sub(r"(\w)-\n(\w)", r"\1\2", text)

    page_patterns = [
        r"(?im)^\s*page\s*[:\-]?\s*\d+\s*(?:/|sur|de|of)\s*\d+\s*$",
        r"(?im)^\s*p(?:age)?\.?\s*[:\-]?\s*\d+\s*$",
        r"(?im)^\s*\d+\s*/\s*\d+\s*$",
        r"(?im)^\s*[-â€“â€”]+\s*\d+\s*[-â€“â€”]+\s*$",
        r"(?im)^\s*\d{1,3}\s*$",
        r"(?im)^\s*page\s+n[Â°Âº]\s*\d+\s*$",
        r"(?im)^\s*page\s*[-â€“â€”]+\s*\d+\s*[-â€“â€”]+\s*$",
    ]
    
    for pattern in page_patterns:
        text = re.sub(pattern, "", text, flags=re.MULTILINE)

    text = re.sub(r"(?m)^\s*\d{1,4}\s*$", "", text)
    text = re.sub(r"(?m)^\s*[â€“â€”\-]{2,}\s*$", "", text)
    text = re.sub(r"(?m)^\s*[-â€“â€”\s]*\s*$", "", text)
    text = re.sub(r"[ \t]+$", "", text, flags=re.MULTILINE)
    text = re.sub(r" {2,}", " ", text)
    text = re.sub(r"(?<![.!?:;])\n(?!\n)(?=[a-zÃ Ã¢Ã¤Ã§Ã©Ã¨ÃªÃ«Ã®Ã¯Ã´Ã¶Ã¹Ã»Ã¼Ã¿Ã±\(\,\;\:\.\-])", " ", text)
    text = re.sub(r"\n{3,}", "\n\n", text)
    
    text = text.strip()
    
    lines = text.split('\n')
    cleaned_lines = []
    for line in lines:
        stripped = line.strip()
        if stripped and not re.match(r'^[\d\s\-â€“â€”\.]{1,10}$', stripped):
            cleaned_lines.append(line)
        elif not stripped:
            cleaned_lines.append(line)
    
    return '\n'.join(cleaned_lines).strip()

def calculate_ocr_stats(text):
    """Calcule les statistiques du texte OCR"""
    if not text:
        return "Texte vide"
    
    lines = text.split('\n')
    words = len(text.split())
    chars = len(text)
    non_empty_lines = len([l for l in lines if l.strip()])
    
    return f"{chars:,} caractÃ¨res | {words:,} mots | {non_empty_lines} lignes non vides | {len(lines)} lignes totales"

# ================
#  CONTRÃ”LE QUALITÃ‰
# ================

def _qc_extract_references(text: str):
    """Extrait des rÃ©fÃ©rences juridiques"""
    if not text:
        return set()
    refs = set()
    t = text.lower()
    
    for m in re.finditer(r"\b(art\.|article)\s+([lrd]\s*[\.\-]?\s*\d+[0-9\-\._]*)", t):
        refs.add(m.group(0).strip())
    
    for m in re.finditer(r"\bcode\s+du\s+(travail|procÃ©dure\s+civile|civil)\b", t):
        refs.add(m.group(0).strip())
    
    return refs

def qc_check_references(ocr_text: str, analysis_text: str) -> str:
    """ContrÃ´le qualitÃ© des rÃ©fÃ©rences"""
    if not analysis_text:
        return "ContrÃ´le qualitÃ© : contenu d'analyse vide."
    
    ocr_refs = _qc_extract_references(ocr_text or "")
    out_refs = _qc_extract_references(analysis_text or "")
    
    if not out_refs:
        return "ContrÃ´le qualitÃ© : aucune rÃ©fÃ©rence dÃ©tectÃ©e dans l'analyse."
    
    missing = sorted(r for r in out_refs if r not in ocr_refs)
    
    if not missing:
        return f"ContrÃ´le qualitÃ© : toutes les rÃ©fÃ©rences citÃ©es ({len(out_refs)}) apparaissent dans le texte OCR."
    
    report = ["ContrÃ´le qualitÃ© : rÃ©fÃ©rences citÃ©es par l'analyse MAIS absentes du texte OCR :"]
    for r in missing:
        report.append(f"  â€¢ {r}")
    report.append(f"\nStatistiques : {len(out_refs)} rÃ©fÃ©rences totales, {len(missing)} manquantes")
    report.append("\nRecommandation : remplacer par Â« non prÃ©cisÃ© dans le document Â» ou reformuler.")
    
    return "\n".join(report)

# ================
#  OLLAMA (API)
# ================

def generate_with_ollama(model: str, prompt_text: str, full_text: str,
                         num_ctx: int, num_predict: int, temperature: float = 0.2,
                         timeout: int = 900) -> str:
    """GÃ©nÃ©ration avec Ollama - SYSTEM PROMPT SÃ‰PARÃ‰"""
    url = "http://localhost:11434/api/generate"
    
    # Construction du payload avec system prompt sÃ©parÃ©
    payload = {
        "model": model,
        "prompt": f"Texte Ã  analyser :\n{full_text}",
        "system": f"{SYSTEM_PROMPT}\n\n{prompt_text}",  # System prompt sÃ©parÃ©
        "stream": True,
        "options": {
            "num_ctx": int(num_ctx),
            "num_predict": int(num_predict),
            "temperature": float(temperature),
        },
    }
    
    try:
        response = requests.post(url, json=payload, stream=True, timeout=timeout)
    except requests.exceptions.ConnectionError:
        return "âŒ Erreur : Impossible de se connecter Ã  Ollama. VÃ©rifiez qu'Ollama est dÃ©marrÃ© (ollama serve)."
    except requests.exceptions.Timeout:
        return f"âŒ Erreur : DÃ©lai dÃ©passÃ© ({timeout}s)."
    except Exception as e:
        return f"âŒ Erreur de connexion Ollama : {e}"
    
    if response.status_code != 200:
        error_text = response.text
        if "system memory" in error_text.lower() and "available" in error_text.lower():
            return f"âŒ MÃ‰MOIRE INSUFFISANTE : Le modÃ¨le nÃ©cessite plus de RAM que disponible.\n\n" \
                   f"Solutions :\n" \
                   f"1. Utilisez un modÃ¨le plus lÃ©ger (mistral:7b-instruct ou deepseek-coder:latest)\n" \
                   f"2. Fermez d'autres applications pour libÃ©rer de la RAM\n" \
                   f"3. RedÃ©marrez Ollama : 'ollama serve'\n\n" \
                   f"Erreur complÃ¨te : {error_text}"
        return f"âŒ Erreur HTTP {response.status_code} : {error_text}"

    parts = []
    try:
        for line in response.iter_lines():
            if not line:
                continue
            try:
                obj = json.loads(line.decode("utf-8"))
                if obj.get("response"):
                    parts.append(obj["response"])
                if obj.get("error"):
                    return f"âŒ Erreur Ollama : {obj['error']}"
            except json.JSONDecodeError:
                continue
    except Exception as e:
        return f"âŒ Erreur lors de la lecture du flux : {e}"
    
    result = "".join(parts).strip()
    return result if result else "âŒ Aucune rÃ©ponse reÃ§ue (flux vide)."

# =================================
#  SAUVEGARDE DES RÃ‰SULTATS
# =================================

def sanitize_filename(name: str) -> str:
    """Nettoie un nom de fichier"""
    name = re.sub(r'[<>:"/\\|?*]', '_', name)
    name = re.sub(r'\s+', '_', name)
    return name[:100]

def save_analysis_result(pdf_name: str, model: str, analysis_text: str, 
                        metadata: dict, format_type: str = "txt") -> tuple:
    """Sauvegarde le rÃ©sultat d'analyse"""
    try:
        script_dir = _script_dir()
        
        # Nom de base du PDF sans extension
        base_name = os.path.splitext(os.path.basename(pdf_name))[0]
        base_name = sanitize_filename(base_name)
        model_clean = sanitize_filename(model)
        
        # Nom du fichier de sortie
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = f"result_{base_name}_{model_clean}_{timestamp}.{format_type}"
        filepath = os.path.join(script_dir, filename)
        
        # Contenu Ã  sauvegarder
        content_parts = []
        
        # En-tÃªte avec mÃ©tadonnÃ©es
        content_parts.append("=" * 80)
        content_parts.append("ANALYSE JURIDIQUE AUTOMATISÃ‰E")
        content_parts.append("=" * 80)
        content_parts.append(f"Document source : {os.path.basename(pdf_name)}")
        content_parts.append(f"Date de traitement : {metadata.get('timestamp', 'N/A')}")
        content_parts.append(f"ModÃ¨le utilisÃ© : {metadata.get('model', 'N/A')}")
        content_parts.append(f"Mode d'analyse : {metadata.get('mode', 'N/A')}")
        content_parts.append(f"Profil d'infÃ©rence : {metadata.get('profil', 'N/A')}")
        content_parts.append(f"Contexte (tokens) : {metadata.get('num_ctx', 'N/A')}")
        content_parts.append(f"Longueur max sortie : {metadata.get('max_tokens', 'N/A')}")
        content_parts.append(f"TempÃ©rature : {metadata.get('temperature', 'N/A')}")
        content_parts.append(f"Temps de traitement : {metadata.get('processing_time', 'N/A')}s")
        content_parts.append(f"Nettoyage avancÃ© : {'Oui' if metadata.get('nettoyer', False) else 'Non'}")
        content_parts.append("")
        
        # Prompt utilisÃ© (tronquÃ©)
        prompt_text = metadata.get('prompt', '')
        if len(prompt_text) > 500:
            prompt_text = prompt_text[:500] + "... [tronquÃ©]"
        content_parts.append("PROMPT UTILISÃ‰ :")
        content_parts.append("-" * 40)
        content_parts.append(prompt_text)
        content_parts.append("")
        
        # RÃ©sultat de l'analyse
        content_parts.append("RÃ‰SULTAT DE L'ANALYSE :")
        content_parts.append("=" * 40)
        content_parts.append(analysis_text)
        
        # Ã‰criture du fichier
        content = "\n".join(content_parts)
        
        if format_type.lower() == "rtf":
            # Format RTF basique
            rtf_content = "{\\rtf1\\ansi\\deff0 {\\fonttbl {\\f0 Times New Roman;}} \\f0\\fs24 " + \
                         content.replace('\n', '\\par ') + "}"
            with open(filepath, 'w', encoding='utf-8') as f:
                f.write(rtf_content)
        else:
            # Format texte
            with open(filepath, 'w', encoding='utf-8') as f:
                f.write(content)
        
        return True, f"RÃ©sultat sauvegardÃ© : {filename}"
        
    except Exception as e:
        return False, f"Erreur sauvegarde : {e}"

# ====================
#  PIPELINE OCR SÃ‰PARÃ‰
# ====================

def do_ocr_only(pdf_path, nettoyer, force_ocr=False):
    """Fait uniquement l'OCR avec cache"""
    if not pdf_path:
        return "âŒ Aucun fichier PDF fourni.", "", ""
    
    if not os.path.exists(pdf_path):
        return "âŒ Fichier PDF introuvable.", "", ""
    
    try:
        pdf_hash = get_pdf_hash(pdf_path)
        ocr_data = None
        
        # Tentative de chargement depuis le cache (sauf si force_ocr=True)
        if pdf_hash and not force_ocr:
            ocr_data = load_ocr_cache(pdf_hash, nettoyer)
        
        if ocr_data:
            # DonnÃ©es OCR trouvÃ©es en cache
            print("âœ… Utilisation du cache OCR existant")
            preview = ocr_data['preview']
            stats = ocr_data['stats']
            total_pages = ocr_data.get('total_pages', '?')
            print(f"Cache utilisÃ© : {total_pages} page(s) dÃ©jÃ  traitÃ©es")
        else:
            # OCR complet nÃ©cessaire
            print(f"ðŸ”„ Conversion PDF : {pdf_path}")
            images = convert_from_path(pdf_path)
            raw_pages = []
            
            total_pages = len(images)
            print(f"ðŸ“„ Traitement de {total_pages} page(s)...")
            
            for i, image in enumerate(images):
                print(f"ðŸ” OCR page {i+1}/{total_pages}...")
                page_text = pytesseract.image_to_string(image, lang="fra")
                raw_pages.append(page_text or "")

            # Nettoyage (TOUJOURS FAIT SI ACTIVÃ‰)
            if nettoyer:
                print("ðŸ§¹ Nettoyage du texte...")
                cleaned_pages = [_normalize_unicode(t) for t in raw_pages]
                preview = smart_clean("\n".join(cleaned_pages), pages_texts=cleaned_pages)
            else:
                preview = "\n".join(raw_pages).strip()

            # Calculer les statistiques OCR
            stats = calculate_ocr_stats(preview)
            
            # Sauvegarder en cache
            if pdf_hash:
                cache_data = {
                    'preview': preview,
                    'stats': stats,
                    'total_pages': total_pages,
                    'timestamp': str(os.path.getmtime(pdf_path))
                }
                save_ocr_cache(pdf_hash, nettoyer, cache_data)

        if not preview.strip():
            return "âŒ Aucun texte dÃ©tectÃ© lors de l'OCR.", stats, preview
        
        return "âœ… OCR terminÃ©. Texte prÃªt pour analyse.", stats, preview

    except Exception as e:
        traceback.print_exc()
        error_msg = f"âŒ Erreur lors de l'OCR : {str(e)}"
        stats = "Erreur - Impossible de calculer les statistiques"
        return error_msg, stats, ""

def do_analysis_only(text_ocr, modele, profil, max_tokens_out, prompt_text, mode_analysis, comparer):
    """Fait uniquement l'analyse juridique"""
    if not text_ocr or not text_ocr.strip():
        return "âŒ Aucun texte OCR disponible pour l'analyse.", "", "", {}
    
    start_time = time.time()
    
    try:
        # VÃ©rification de la longueur du texte
        text_length = len(text_ocr)
        estimated_tokens = text_length // 4
        
        print(f"ðŸ“Š Longueur du texte : {text_length:,} caractÃ¨res (â‰ˆ{estimated_tokens:,} tokens)")
        
        if estimated_tokens > 20000:
            print("âš ï¸ Document trÃ¨s volumineux dÃ©tectÃ© - recommandation profil 'Maxi'")
        elif estimated_tokens > 10000:
            print("âš ï¸ Document volumineux - recommandation profil 'Confort' ou 'Maxi'")

        # Configuration du profil
        profiles = {
            "Rapide":  {"num_ctx": 8192,  "temperature": 0.2},
            "Confort": {"num_ctx": 16384, "temperature": 0.2},
            "Maxi":    {"num_ctx": 32768, "temperature": 0.2},
        }
        base = profiles.get(profil, profiles["Confort"])
        num_ctx = base["num_ctx"]
        temperature = base["temperature"]
        
        # Avertissement si le contexte pourrait Ãªtre insuffisant
        if estimated_tokens > num_ctx * 0.8:
            print(f"âš ï¸ ATTENTION : Le texte ({estimated_tokens:,} tokens) approche la limite du contexte ({num_ctx:,})")
            print("ConsidÃ©rez utiliser le profil 'Maxi' pour de meilleurs rÃ©sultats")

        # SÃ©lection du prompt principal
        main_prompt = EXPERT_PROMPT_TEXT if (mode_analysis or "").lower().startswith("expert") else prompt_text

        print(f"ðŸ¤– GÃ©nÃ©ration avec {modele} en mode {mode_analysis}...")
        
        # Analyse principale
        analyse = generate_with_ollama(
            modele, main_prompt, text_ocr,
            num_ctx=num_ctx, num_predict=max_tokens_out, temperature=temperature
        )

        # Temps de traitement
        processing_time = round(time.time() - start_time, 2)

        # MÃ©tadonnÃ©es pour la sauvegarde
        metadata = {
            'timestamp': datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            'model': modele,
            'mode': mode_analysis,
            'profil': profil,
            'num_ctx': num_ctx,
            'max_tokens': max_tokens_out,
            'temperature': temperature,
            'processing_time': processing_time,
            'prompt': main_prompt
        }

        # Ajout des paramÃ¨tres dans la sortie
        params_info = f"""
=== PARAMÃˆTRES D'ANALYSE ===
ModÃ¨le : {modele}
Mode : {mode_analysis}
Profil : {profil} (contexte: {num_ctx}, tempÃ©rature: {temperature})
Longueur max : {max_tokens_out} tokens
Temps de traitement : {processing_time}s
Date : {metadata['timestamp']}
========================

"""

        analyse_avec_params = params_info + analyse

        qc_or_compare = ""
        analyse_alt = ""

        # ContrÃ´le qualitÃ© en mode Expert
        if (mode_analysis or "").lower().startswith("expert"):
            print("ðŸ” ContrÃ´le qualitÃ©...")
            qc_or_compare = qc_check_references(text_ocr, analyse)

        # Comparaison optionnelle
        if comparer:
            print("âš–ï¸ GÃ©nÃ©ration comparative...")
            alt_prompt = DEFAULT_PROMPT_TEXT if (mode_analysis or "").lower().startswith("expert") else EXPERT_PROMPT_TEXT
            analyse_alt = generate_with_ollama(
                modele, alt_prompt, text_ocr,
                num_ctx=num_ctx, num_predict=max_tokens_out, temperature=temperature
            )
            
            # Statistiques de comparaison
            def _summary_diff(a, b):
                if not a or not b or a.startswith("âŒ"):
                    return "Comparaison impossible (erreur ou contenu vide)."
                
                words_a = set(w.lower() for w in re.findall(r"\b\w{4,}\b", a))
                words_b = set(w.lower() for w in re.findall(r"\b\w{4,}\b", b))
                
                if not words_a and not words_b:
                    return "Aucun mot significatif dÃ©tectÃ©."
                
                intersection = len(words_a & words_b)
                union = len(words_a | words_b) or 1
                jaccard = intersection / union
                
                return (f"SimilaritÃ© lexicale : {jaccard:.2%}\n"
                       f"Mots uniques analyse 1 : {len(words_a)}\n"
                       f"Mots uniques analyse 2 : {len(words_b)}\n"
                       f"Mots communs : {intersection}")
            
            comp = _summary_diff(analyse, analyse_alt)
            qc_or_compare = (qc_or_compare + "\n\n" + comp).strip() if qc_or_compare else comp

        # Retourner aussi les mÃ©tadonnÃ©es pour la sauvegarde
        return analyse_avec_params, analyse_alt, qc_or_compare, metadata

    except Exception as e:
        traceback.print_exc()
        error_msg = f"âŒ Erreur lors de l'analyse : {str(e)}"
        return error_msg, "", "", {}

# ====================
#  MODELES OLLAMA
# ====================

def get_ollama_models():
    """RÃ©cupÃ¨re la liste des modÃ¨les avec prioritÃ© aux plus lÃ©gers"""
    fallback_models = [
        "mistral:7b-instruct",
        "mistral:latest", 
        "deepseek-coder:latest",
        "llama3:latest",
        "llama3.1:8b-instruct-q5_K_M"
    ]
    
    try:
        print("ðŸ” Tentative de rÃ©cupÃ©ration des modÃ¨les Ollama...")
        r = requests.get(f"{api_url_input}/api/tags", timeout=10)
        
        if r.status_code != 200:
            print(f"âŒ Erreur API Ollama - Status {r.status_code}: {r.text}")
            return fallback_models
        
        data = r.json()
        models = data.get("models", [])
        names = []
        
        for m in models:
            name = m.get("name")
            if name:
                names.append(name)
        
        if not names:
            print("âš ï¸ Aucun modÃ¨le trouvÃ© dans la rÃ©ponse, utilisation des modÃ¨les de fallback")
            return fallback_models
        
        print(f"âœ… Total: {len(names)} modÃ¨le(s) rÃ©cupÃ©rÃ©(s) depuis l'API")
        return names
        
    except requests.exceptions.ConnectionError:
        print("âŒ Ollama non accessible (connexion refusÃ©e)")
        print("VÃ©rifiez qu'Ollama est dÃ©marrÃ© avec: ollama serve")
        return fallback_models
    except Exception as e:
        print(f"âŒ Erreur rÃ©cupÃ©ration modÃ¨les: {e}")
        return fallback_models

# ===========
#  UI GRADIO
# ===========

def build_ui():
    """Construit l'interface utilisateur Gradio"""
    models_list = get_ollama_models()
    store = load_prompt_store()
    prompt_names = [DEFAULT_PROMPT_NAME] + sorted([n for n in store.keys() if n != DEFAULT_PROMPT_NAME])
    
    script_name = os.path.basename(__file__) if '__file__' in globals() else "cph_fixed.py"

    with gr.Blocks(title=f"{script_name} - OCR Juridique + Ollama") as demo:

        with gr.Row():
            api_url_input = gr.Textbox(label="URL API Ollama", value="http://localhost:11434", interactive=True)
            api_url_state = gr.State(value="http://localhost:11434")

        gr.Markdown("## OCR structurÃ© + Analyse juridique (Ollama)")
        gr.Markdown(f"**Fichier des prompts** : `{PROMPT_STORE_PATH}`")

        with gr.Row():
            pdf_file = gr.File(label="Uploader un PDF scannÃ©", file_types=[".pdf"])
            nettoyer = gr.Checkbox(label="Nettoyage avancÃ©", value=True)

        with gr.Row():
            force_ocr = gr.Checkbox(label="Forcer nouveau OCR (ignorer le cache)", value=False)
            clear_cache_btn = gr.Button("Vider le cache OCR", variant="secondary", size="sm")
            cache_info = gr.Markdown("")

        with gr.Row():
            if "mistral:7b-instruct" in models_list:
                default_model = "mistral:7b-instruct"
            elif "deepseek-coder:latest" in models_list:
                default_model = "deepseek-coder:latest"
            elif "mistral:latest" in models_list:
                default_model = "mistral:latest"
            else:
                default_model = models_list[0] if models_list else "mistral:latest"
                
            modele = gr.Dropdown(label="ModÃ¨le Ollama", choices=models_list, value=default_model)
            profil = gr.Radio(label="Profil", choices=["Rapide", "Confort", "Maxi"], value="Confort")
            max_tokens_out = gr.Slider(label="Longueur (tokens)", minimum=256, maximum=2048, step=128, value=1280)
        
        with gr.Row():
            mode_analysis = gr.Radio(label="Mode", choices=["Standard", "Expert"], value="Standard")
            comparer = gr.Checkbox(label="Comparer avec l'autre mode", value=False)

        gr.Markdown("### Prompt â€” gestion persistante (JSON)")

        with gr.Row():
            prompt_selector = gr.Dropdown(label="Choisir un prompt", choices=prompt_names, value=DEFAULT_PROMPT_NAME)
            prompt_name_box = gr.Textbox(label="Nom du prompt", lines=1, placeholder="Nom...")

        prompt_box = gr.Textbox(
            label="Contenu du prompt (modifiable)",
            value=store.get(DEFAULT_PROMPT_NAME, DEFAULT_PROMPT_TEXT),
            lines=12,
            interactive=True
        )

        with gr.Row():
            save_btn = gr.Button("Enregistrer", variant="secondary")
            save_as_btn = gr.Button("Enregistrer sous...", variant="secondary")
            rename_btn = gr.Button("Renommer", variant="secondary")
            delete_btn = gr.Button("Supprimer", variant="secondary")
            reset_btn = gr.Button("RÃ©initialiser", variant="secondary")
            reload_btn = gr.Button("Recharger", variant="secondary")

        info_box = gr.Markdown("")

        gr.Markdown("### Compositeur de prompts (fichiers prompt*.txt)")
        
        prompt_files = get_prompt_files(PROMPT_FILES_DIR)
        
        with gr.Row():
            files_checklist = gr.CheckboxGroup(
                label="Fichiers prompt*.txt disponibles dans ./prompts/",
                choices=prompt_files,
                value=[]
            )
            refresh_files_btn = gr.Button("Actualiser fichiers", variant="secondary")
        
        with gr.Row():
            compose_btn = gr.Button("Composer prompt gÃ©nÃ©ral", variant="primary")
            copy_to_main_btn = gr.Button("Copier vers prompt principal", variant="secondary")
        
        composed_prompt_box = gr.Textbox(
            label="Prompt gÃ©nÃ©ral composÃ©",
            lines=15,
            interactive=False,
            show_copy_button=True,
            placeholder="Le prompt composÃ© apparaÃ®tra ici..."
        )

        # Boutons principaux
        with gr.Row():
            ocr_btn = gr.Button("1. OCR seulement", variant="secondary")
            analyze_btn = gr.Button("2. Analyser", variant="primary", size="lg")
            full_btn = gr.Button("OCR + Analyse", variant="primary")

        with gr.Row():
            save_result_btn = gr.Button("Sauvegarder rÃ©sultat", variant="secondary")
            save_format = gr.Radio(label="Format", choices=["txt", "rtf"], value="txt")
            save_status = gr.Markdown("")

        with gr.Tabs():
            with gr.Tab("Analyse (mode choisi)"):
                analysis_box = gr.Textbox(label="Analyse juridique", lines=36, show_copy_button=True)
            with gr.Tab("Analyse (autre mode)"):
                analysis_alt_box = gr.Textbox(label="Analyse comparative", lines=24, show_copy_button=True)
            with gr.Tab("ContrÃ´le qualitÃ©"):
                compare_box = gr.Textbox(label="Rapport CQ & comparatif", lines=18, show_copy_button=True)
            with gr.Tab("Texte OCR"):
                ocr_stats = gr.Textbox(label="Statistiques", lines=2, interactive=False)
                preview_box = gr.Textbox(label="Texte OCR extrait", interactive=False, show_copy_button=True, lines=25)

        # Ã‰tats
        prompts_state = gr.State(value=store)
        current_name = gr.State(value=DEFAULT_PROMPT_NAME)
        current_ocr_text = gr.State(value="")  # Pour stocker le texte OCR
        current_pdf_path = gr.State(value="")  # Pour stocker le chemin PDF
        analysis_metadata = gr.State(value={})  # Pour les mÃ©tadonnÃ©es d'analyse

        # === FONCTIONS CALLBACKS ===
        
        def clear_cache():
            count = clear_ocr_cache()
            if count > 0:
                return gr.Markdown.update(value=f"Cache vidÃ© : {count} fichier(s) supprimÃ©(s)")
            else:
                return gr.Markdown.update(value="Cache dÃ©jÃ  vide")

        def launch_ocr_only(pdf_path, nettoyer, force_ocr):
            """Lance seulement l'OCR et charge le rÃ©sultat dans l'onglet OCR"""
            try:
                message, stats, preview = do_ocr_only(pdf_path, nettoyer, force_ocr)
                return (
                    message,  # analysis_box (message de statut)
                    "",       # analysis_alt_box  
                    "",       # compare_box
                    stats,    # ocr_stats
                    preview,  # preview_box
                    preview,  # current_ocr_text (state)
                    pdf_path if pdf_path else ""  # current_pdf_path (state)
                )
            except Exception as e:
                traceback.print_exc()
                return f"âŒ Erreur OCR : {str(e)}", "", "", "Erreur", "", "", ""

        def launch_analysis_only(ocr_text, pdf_path, modele, profil, max_tokens_out, 
                                prompt_text, mode_analysis, comparer, nettoyer):
            """Lance seulement l'analyse juridique avec le texte OCR disponible"""
            try:
                # Si pas de texte OCR ET pas de PDF, erreur
                if not ocr_text and not pdf_path:
                    return "âŒ Aucun texte OCR ni fichier PDF disponible.", "", "", "", "", "", {}
                
                # Si pas de texte OCR mais PDF disponible, faire d'abord l'OCR
                if not ocr_text and pdf_path:
                    message, stats, preview = do_ocr_only(pdf_path, nettoyer, False)
                    if "âŒ" in message:
                        return message, "", "", stats, preview, preview, {}
                    ocr_text = preview
                
                # Faire l'analyse
                analyse, analyse_alt, qc_compare, metadata = do_analysis_only(
                    ocr_text, modele, profil, max_tokens_out, prompt_text, mode_analysis, comparer
                )
                
                # Ajouter les paramÃ¨tres de nettoyage aux mÃ©tadonnÃ©es
                metadata['nettoyer'] = nettoyer
                
                stats = calculate_ocr_stats(ocr_text)
                
                return (
                    analyse,     # analysis_box
                    analyse_alt, # analysis_alt_box  
                    qc_compare,  # compare_box
                    stats,       # ocr_stats
                    ocr_text,    # preview_box
                    ocr_text,    # current_ocr_text (state)
                    metadata     # analysis_metadata (state)
                )
                
            except Exception as e:
                traceback.print_exc()
                return f"âŒ Erreur analyse : {str(e)}", "", "", "Erreur", "", "", {}

        def launch_full_pipeline(pdf_path, nettoyer, force_ocr, modele, profil, max_tokens_out, 
                               prompt_text, mode_analysis, comparer):
            """Pipeline complet OCR + Analyse"""
            try:
                if not pdf_path:
                    return "âŒ Aucun fichier PDF fourni.", "", "", "", "", "", {}
                
                # Ã‰tape 1: OCR
                message, stats, ocr_text = do_ocr_only(pdf_path, nettoyer, force_ocr)
                if "âŒ" in message:
                    return message, "", "", stats, ocr_text, ocr_text, {}
                
                # Ã‰tape 2: Analyse
                analyse, analyse_alt, qc_compare, metadata = do_analysis_only(
                    ocr_text, modele, profil, max_tokens_out, prompt_text, mode_analysis, comparer
                )
                
                metadata['nettoyer'] = nettoyer
                
                return (
                    analyse,     # analysis_box
                    analyse_alt, # analysis_alt_box
                    qc_compare,  # compare_box
                    stats,       # ocr_stats
                    ocr_text,    # preview_box
                    ocr_text,    # current_ocr_text (state)
                    metadata     # analysis_metadata (state)
                )
                
            except Exception as e:
                traceback.print_exc()
                return f"âŒ Erreur pipeline : {str(e)}", "", "", "Erreur", "", "", {}

        def save_analysis_file(pdf_path, analysis_text, metadata, format_type):
            """Sauvegarde le rÃ©sultat d'analyse"""
            if not analysis_text or not pdf_path:
                return "âŒ Aucun rÃ©sultat Ã  sauvegarder"
            
            model = metadata.get('model', 'unknown')
            success, message = save_analysis_result(pdf_path, model, analysis_text, metadata, format_type)
            
            if success:
                return f"âœ… {message}"
            else:
                return f"âŒ {message}"

        def on_pdf_upload(pdf_path, nettoyer):
            """Charge automatiquement l'OCR si cache disponible"""
            if not pdf_path:
                return "", "", "", ""
            
            try:
                pdf_hash = get_pdf_hash(pdf_path)
                if pdf_hash:
                    ocr_data = load_ocr_cache(pdf_hash, nettoyer)
                    if ocr_data:
                        preview = ocr_data['preview']
                        stats = ocr_data['stats']
                        return stats, preview, preview, pdf_path
                
                return "", "", "", pdf_path
            except:
                return "", "", "", pdf_path

        # Gestion prompts JSON (fonctions simplifiÃ©es)
        def on_select(name, store):
            try:
                name = sanitize_name(name) or DEFAULT_PROMPT_NAME
                if name not in store:
                    name = DEFAULT_PROMPT_NAME
                text = store.get(name, DEFAULT_PROMPT_TEXT)
                return (
                    gr.Textbox.update(value=text),
                    gr.Textbox.update(value=name),
                    name,
                    gr.Markdown.update(value=f"SÃ©lectionnÃ© : **{name}**")
                )
            except Exception as e:
                return (
                    gr.Textbox.update(value=DEFAULT_PROMPT_TEXT),
                    gr.Textbox.update(value=DEFAULT_PROMPT_NAME),
                    DEFAULT_PROMPT_NAME,
                    gr.Markdown.update(value=f"Erreur : {e}")
                )

        def on_save(current_name_val, text, store):
            try:
                name = sanitize_name(current_name_val) or DEFAULT_PROMPT_NAME
                if not text.strip():
                    return (
                        gr.Dropdown.update(),
                        gr.Markdown.update(value="Le contenu ne peut pas Ãªtre vide."),
                        store,
                        current_name_val
                    )
                
                store[name] = text.strip()
                ok, msg = save_prompt_store(store)
                
                if ok:
                    names = [DEFAULT_PROMPT_NAME] + sorted([n for n in store.keys() if n != DEFAULT_PROMPT_NAME])
                    return (
                        gr.Dropdown.update(choices=names, value=name),
                        gr.Markdown.update(value=f"âœ… EnregistrÃ© : {msg}"),
                        store,
                        name
                    )
                else:
                    return (
                        gr.Dropdown.update(),
                        gr.Markdown.update(value=f"âŒ {msg}"),
                        store,
                        current_name_val
                    )
            except Exception as e:
                return (
                    gr.Dropdown.update(),
                    gr.Markdown.update(value=f"âŒ Erreur : {e}"),
                    store,
                    current_name_val
                )

        def refresh_prompt_files():
            files = get_prompt_files(PROMPT_FILES_DIR)
            return gr.CheckboxGroup.update(choices=files, value=[])

        def compose_selected_prompts(selected_files):
            if not selected_files:
                return "Aucun fichier sÃ©lectionnÃ©."
            return concatenate_prompts(selected_files, PROMPT_FILES_DIR)

        def copy_composed_to_main(composed_text):
            """Copie le prompt composÃ© vers le prompt principal"""
            if not composed_text or "Aucun fichier" in composed_text:
                return ("", "Aucun prompt composÃ© Ã  copier.")
            
            lines = composed_text.split('\n')
            clean_lines = []
            
            for line in lines:
                if line.startswith("=== ") or line.startswith("--- "):
                    continue
                clean_lines.append(line)
            
            clean_prompt = '\n'.join(clean_lines).strip()
            
            if not clean_prompt:
                return ("", "Prompt vide aprÃ¨s nettoyage.")
            
            return (clean_prompt, f"âœ… Prompt composÃ© copiÃ© ({len(clean_prompt)} caractÃ¨res).")

        # === CONNEXIONS ===
        
        pdf_file.change(
            fn=on_pdf_upload,
            inputs=[pdf_file, nettoyer],
            outputs=[ocr_stats, preview_box, current_ocr_text, current_pdf_path]
        )
        
        prompt_selector.change(
            fn=on_select,
            inputs=[prompt_selector, prompts_state],
            outputs=[prompt_box, prompt_name_box, current_name, info_box]
        )

        save_btn.click(
            fn=on_save,
            inputs=[current_name, prompt_box, prompts_state],
            outputs=[prompt_selector, info_box, prompts_state, current_name]
        )

        refresh_files_btn.click(
            fn=refresh_prompt_files,
            outputs=[files_checklist]
        )

        compose_btn.click(
            fn=compose_selected_prompts,
            inputs=[files_checklist],
            outputs=[composed_prompt_box]
        )

        copy_to_main_btn.click(
            fn=copy_composed_to_main,
            inputs=[composed_prompt_box],
            outputs=[prompt_box, info_box]
        )

        # Boutons principaux
        ocr_btn.click(
            fn=launch_ocr_only,
            inputs=[pdf_file, nettoyer, force_ocr],
            outputs=[analysis_box, analysis_alt_box, compare_box, ocr_stats, preview_box, current_ocr_text, current_pdf_path]
        )

        analyze_btn.click(
            fn=launch_analysis_only,
            inputs=[current_ocr_text, current_pdf_path, modele, profil, max_tokens_out, 
                   prompt_box, mode_analysis, comparer, nettoyer],
            outputs=[analysis_box, analysis_alt_box, compare_box, ocr_stats, preview_box, current_ocr_text, analysis_metadata]
        )

        full_btn.click(
            fn=launch_full_pipeline,
            inputs=[pdf_file, nettoyer, force_ocr, modele, profil, max_tokens_out, 
                   prompt_box, mode_analysis, comparer],
            outputs=[analysis_box, analysis_alt_box, compare_box, ocr_stats, preview_box, current_ocr_text, analysis_metadata]
        )

        save_result_btn.click(
            fn=save_analysis_file,
            inputs=[current_pdf_path, analysis_box, analysis_metadata, save_format],
            outputs=[save_status]
        )

        clear_cache_btn.click(
            fn=clear_cache,
            outputs=[cache_info]
        )

        gr.Markdown("""
        ### Guide d'utilisation

        **Flux de travail recommandÃ© :**
        1. **Uploader un PDF** - Le cache OCR se charge automatiquement si disponible
        2. **OCR seulement** - Traite le PDF et met en cache le rÃ©sultat
        3. **Analyser** - Lance l'analyse juridique sur le texte OCR disponible
        4. **Sauvegarder** - Exporte le rÃ©sultat avec mÃ©tadonnÃ©es

        **ModÃ¨les recommandÃ©s (selon RAM) :**
        - **8 GB ou moins** : mistral:7b-instruct, deepseek-coder:latest
        - **12+ GB** : llama3:latest, mistral:latest
        - **16+ GB** : llama3.1:8b-instruct-q5_K_M

        **Profils d'infÃ©rence :**
        - **Rapide** : Documents < 15 pages (8k contexte)
        - **Confort** : Documents 15-30 pages (16k contexte)  
        - **Maxi** : Documents 30+ pages (32k contexte)

        **PrÃ©requis** : Ollama dÃ©marrÃ© (`ollama serve`)  
        **RÃ©pertoires** : Cache `./cache_ocr/` | Prompts `./prompts/`
        """)

    return demo

# ========================
#  POINT D'ENTRÃ‰E PRINCIPAL
# ========================

def main():
    """Point d'entrÃ©e principal"""
    parser = argparse.ArgumentParser(description="OCR + Analyse juridique avec Ollama")
    parser.add_argument('--list-models', action='store_true', help='Lister les modÃ¨les Ollama disponibles')
    parser.add_argument('--gui', action='store_true', help='Lancer l\'interface graphique (dÃ©faut)')
    parser.add_argument('--host', default='127.0.0.1', help='Adresse d\'Ã©coute (dÃ©faut: 127.0.0.1)')
    parser.add_argument('--port', type=int, help='Port d\'Ã©coute (dÃ©faut: auto)')
    
    args = parser.parse_args()
    
    if args.list_models:
        print("ModÃ¨les Ollama disponibles:")
        try:
            models = get_ollama_models()
            for i, model in enumerate(models, 1):
                print(f"  {i}. {model}")
        except Exception as e:
            print(f"Erreur: {e}")
        return
    
    # Interface graphique par dÃ©faut
    print("ðŸš€ DÃ©marrage de l'interface OCR Juridique...")
    print(f"ðŸ“ RÃ©pertoire des prompts : {PROMPT_STORE_DIR}")
    
    if not os.path.exists(PROMPT_STORE_DIR):
        os.makedirs(PROMPT_STORE_DIR, exist_ok=True)
    
    try:
        models = get_ollama_models()
        print(f"ðŸ¤– ModÃ¨les disponibles : {len(models)}")
        
        app = build_ui()
        
        launch_kwargs = {
            'server_name': args.host,
            'share': False,
            'inbrowser': True
        }
        
        if args.port:
            launch_kwargs['server_port'] = args.port
            
        app.launch(**launch_kwargs)
        
    except Exception as e:
        print(f"âŒ Erreur : {e}")
        traceback.print_exc()
        sys.exit(1)

if __name__ == "__main__":
    main()