...
def get_cache_path(pdf_hash: str, nettoyer: bool) -> str:
    ...
    cache_dir = os.path.join("cache_ocr")
...
def setup_argparser():
    ...
    parser = argparse.ArgumentParser(
        description="OCR + Analyse juridique automatisée avec Ollama",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Exemples d'usage:
  %(prog)s --gui                                    # Interface graphique (défaut)
  %(prog)s -i document.pdf -m mistral:7b-instruct  # Analyse simple
  %(prog)s -i doc.pdf -m mistral,llama3 -o results # Analyse multi-modèles
  %(prog)s -i doc.pdf -p "prompt custom" --clean   # Avec prompt personnalisé
  %(prog)s -i doc.pdf --prompt-file prompt1.txt    # Avec fichier prompt
        """
    )
    
    # Mode d'exécution
    parser.add_argument('--gui', action='store_true', default=False,
                       help='Lancer l\'interface graphique (défaut si aucun argument CLI)')
    ...
    parser.add_argument('-m', '--models', type=str,
                       help='Modèle(s) Ollama à utiliser (séparés par des virgules)')
    
    # Prompts
    parser.add_argument('-p', '--prompt', type=str,
                       help='Prompt personnalisé (texte direct)')
    parser.add_argument('--prompt-file', type=str,
                       help='Fichier contenant le prompt à utiliser')
    parser.add_argument('--prompt-compose', type=str,
                       help='Fichiers prompt*.txt à composer (séparés par des virgules)')
    
    # Options OCR
    parser.add_argument('--clean', action='store_true', default=True,
                       help='Activer le nettoyage avancé OCR (défaut)')
    parser.add_argument('--no-clean', action='store_true',
                       help='Désactiver le nettoyage avancé OCR')
    parser.add_argument('--force-ocr', action='store_true',
                       help='Forcer nouveau OCR (ignorer le cache)')
    
    # Paramètres d'analyse
    parser.add_argument('--mode', choices=['standard', 'expert'], default='standard',
                       help='Mode d\'analyse (défaut: standard)')
    parser.add_argument('--profile', choices=['rapide', 'confort', 'maxi'], default='confort',
                       help='Profil d\'inférence (défaut: confort)')
    parser.add_argument('--max-tokens', type=int, default=1280,
                       help='Longueur maximale de sortie en tokens (défaut: 1280)')
    parser.add_argument('--compare', action='store_true',
                       help='Activer la comparaison standard vs expert')
    
    # Format de sortie
    parser.add_argument('--format', choices=['txt', 'rtf'], default='txt',
                       help='Format de sortie (défaut: txt)')
    parser.add_argument('--no-metadata', action='store_true',
                       help='Ne pas inclure les métadonnées dans la sortie')
    
    # Autres options
    parser.add_argument('--verbose', '-v', action='store_true',
                       help='Mode verbeux')
    parser.add_argument('--list-models', action='store_true',
                       help='Lister les modèles Ollama disponibles et quitter')
    parser.add_argument('--list-prompts', action='store_true',
                       help='Lister les fichiers prompts disponibles et quitter')
...
def cli_list_prompts():
    """Liste les fichiers prompts disponibles"""
    print(f"Fichiers prompts dans {PROMPT_FILES_DIR}:")
    ...
    # Fichiers prompt*.txt
    txt_files = get_prompt_files(PROMPT_FILES_DIR)
    if txt_files:
        print("\nFichiers prompt*.txt:")
        for filename in txt_files:
            print(f"  - {filename}")
    
    if not store and not txt_files:
        print("  Aucun prompt trouvé")

def cli_load_prompt(args):
    """Charge le prompt selon les arguments CLI"""
    # Priorité: --prompt > --prompt-file > --prompt-compose > défaut
    ...
    if args.prompt_file:
        try:
            if not os.path.isabs(args.prompt_file):
                prompt_path = os.path.join(PROMPT_FILES_DIR, args.prompt_file)
            else:
            ...
            print(f"Erreur lecture prompt-file '{args.prompt_file}': {e}")
            sys.exit(1)
    
    if args.prompt_compose:
        files = [f.strip() for f in args.prompt_compose.split(',')]
        return concatenate_prompts(files, PROMPT_FILES_DIR)
    
    # Défaut
    if args.mode == 'expert':
        return EXPERT_PROMPT_TEXT
    else:
        return DEFAULT_PROMPT_TEXT

def cli_process_file(args):
    ...
    if not args.models:
        print("Erreur: Aucun modèle spécifié (-m/--models requis en mode CLI)")
        sys.exit(1)
    
    # Préparation
    models = [m.strip() for m in args.models.split(',')]
    prompt_text = cli_load_prompt(args)
    clean_ocr = args.clean and not args.no_clean
    ...
    if args.verbose:
        ...
        print(f"Nettoyage OCR: {clean_ocr}")
    ...
    message, stats, ocr_text = do_ocr_only(args.input, clean_ocr, args.force_ocr)
    
    if "Erreur" in message:
        print(f"Erreur OCR: {message}")
        sys.exit(1)
    
    if args.verbose:
        print(f"OCR terminé: {stats}")
    ...
    for model in models:
        ...
        try:
            analyse, analyse_alt, qc_compare, metadata = do_analysis_only(
                ocr_text, model, args.profile.title(), args.max_tokens, 
                prompt_text, args.mode.title(), args.compare
            )
            
            processing_time = round(time.time() - start_time, 2)
            
            if analyse.startswith("Erreur"):
                print(f"Erreur analyse {model}: {analyse}")
                continue
            ...
            results.append({
                'model': model,
                'analysis': analyse,
                'analysis_alt': analyse_alt,
                'qc_compare': qc_compare,
                'metadata': metadata
            })
            
            if args.verbose:
                print(f"Analyse terminée en {processing_time}s")
        
        except Exception as e:
            print(f"Erreur analyse {model}: {e}")
            continue
    ...
    for result in results:
        ...
        model_name = sanitize_filename(result['model'])
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        
        output_name = f"result_{base_name}_{model_name}_{timestamp}.{args.format}"
        ...
        if args.no_metadata:
            ...
            content_parts = []
            content_parts.append("=" * 80)
            content_parts.append("ANALYSE JURIDIQUE AUTOMATISÉE - MODE CLI")
            content_parts.append("=" * 80)
            ...
            content_parts.append("RÉSULTAT DE L'ANALYSE :")
            content_parts.append("=" * 40)
            content_parts.append(result['analysis'])
            
            if result['analysis_alt']:
                content_parts.append("")
                content_parts.append("ANALYSE COMPARATIVE :")
                content_parts.append("=" * 40)
                content_parts.append(result['analysis_alt'])
            
            if result['qc_compare']:
                content_parts.append("")
                content_parts.append("CONTRÔLE QUALITÉ & COMPARATIF :")
                content_parts.append("=" * 40)
                content_parts.append(result['qc_compare'])
    ...
    # Résumé final
    print(f"\nTraitement terminé:")
    print(f"  - Fichier traité: {args.input}")
    print(f"  - Modèles utilisés: {len(results)}/{len(models)}")
...
def main():
    ...
    # Arguments CLI
    parser.add_argument('--input', '-i', help='Fichier PDF d\'entrée')
    parser.add_argument('--models', '-m', help='Modèles à utiliser (séparés par virgules)')
    parser.add_argument('--output', '-o', default='.', help='Répertoire de sortie')
    parser.add_argument('--verbose', '-v', action='store_true', help='Mode verbeux')
    parser.add_argument('--clean', action='store_true', default=True, help='Nettoyage avancé (défaut)')
    parser.add_argument('--no-clean', action='store_true', help='Désactiver le nettoyage')
    parser.add_argument('--mode', choices=['standard', 'expert'], default='standard', help='Mode d\'analyse')
    ...
    # Mode CLI si --input et --models sont fournis
    if args.input and args.models:
        print("Mode CLI - traitement en cours...")
        print(f"Fichier: {args.input}")
        print(f"Modèles: {args.models}")
        print(f"Sortie: {args.output}")
        print("ATTENTION: Fonctionnalité CLI en développement - utilisez l'interface graphique pour l'instant")
        return
