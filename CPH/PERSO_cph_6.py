...
PATCHES DANS CETTE VERSION :
- [PATCH] Regex téléphone FR plus robuste dans _anonymize_phone_numbers()
- [PATCH] Ping initial des modèles dans main() protégé par try/except
"""
import os
import sys
import re
...
class TextAnonymizer:
    ...
    def _anonymize_phone_numbers(self, text: str) -> str:
        # [PATCH] Regex FR robuste : 0X XX XX XX XX ou +33 X XX XX XX XX, avec séparateurs espace/.- facultatifs
        phone_pattern = r"\b(?:0|\+33 ?)(?:1-9{4})\b"
        return re.sub(phone_pattern, '[TELEPHONE_ANONYME]', text)
...
def get_ollama_models(api_base: Optional[str] = None) -> List[str]:
    base = (api_base or get_saved_api_url()).strip().rstrip("/")
    fallback = [
        "mistral:7b-instruct",
        "mistral:latest",
    ...
    try:
        base = (api_base or get_saved_api_url()).strip().rstrip("/")
        print("🔎 Récupération des modèles Ollama…")
        r = requests.get(f"{base}/api/tags", timeout=10)
        if r.status_code != 200:
            print(f"❌ Erreur API Ollama ({r.status_code}): {r.text}")
            return fallback
        data = r.json()
        models = data.get("models", [])
        names = [m.get("name") for m in models if m.get("name")]
        if not names:
            print("⚠️ Aucun modèle dans la réponse, fallback.")
            return fallback
        print(f"✅ {len(names)} modèle(s) trouvés.")
        return names
    except requests.exceptions.ConnectionError:
        print("❌ Ollama non accessible (connexion refusée). Démarrez 'ollama serve'.")
        return fallback
    except Exception as e:
...
def main():
    ...
    try:
        ...
        # [PATCH] Ping initial protégé : si l'API n'est pas joignable, on continue quand même
        try:
            _ = get_ollama_models(initial_url)  # ping initial (logs console)
        except Exception as e:
            print(f"⚠️ Impossible de récupérer les modèles Ollama : {e}")
